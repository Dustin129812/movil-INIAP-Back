...
class LoteResource extends JsonResource
{
    public function toArray($request): array
    {
        ...
        // Mapear vertices desde geometria
        $vertices = [];
        $verticesCount = 0;
        if ($this->geometria_geojson) {
            $geo = json_decode($this->geometria_geojson, true);
            if (isset($geo['coordinates'][0])) {
                $vertices = $geo['coordinates'][0];
                $verticesCount = count($vertices);
                // Remover el último si es igual al primero (cerrado)
                if ($verticesCount > 0 && $vertices[0] === $vertices[$verticesCount - 1]) {
                    array_pop($vertices);
                    $verticesCount--;
                }
            }
        ...
        // sync_status: PENDING si pendiente, SYNCED si verificado
        $syncStatus = ($this->estado_verificacion === 'verificado') ? 'SYNCED' : 'PENDING';

        return [
            'id' => $this->id,
            'uuid_movil' => $this->uuid_movil,
            'sync_status' => $syncStatus,
            'estado_verificacion' => $this->estado_verificacion ?? 'pendiente',
            ...
            'estado' => $this->estado_verificacion ?? 'pendiente',
            'ubicacion_manual' => $this->ubicacion_manual,
            'vertices' => $vertices,
